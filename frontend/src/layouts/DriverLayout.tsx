...
export default function DriverLayout() {
  ...
  const handleModeSwitch = (targetMode: "RIDER" | "DRIVER") => {
    if (targetMode === "RIDER") {
      toast.success("Rider mode enabled");
      navigate("/dashboard");
    }
  };
  ...
  const desktopNavItems = [
    ...
    { label: "My Trips", to: "/driver/trips", icon: MapPinned },
    { label: "Offer a Ride", to: "/driver/trips/create", icon: PlusCircle },
    { label: "Requests", to: "/driver/bookings", icon: CalendarRange },
  ...
  const mobileNavItems = [
    { label: "Home", to: "/driver/dashboard", icon: Home },
    { label: "My Trips", to: "/driver/trips", icon: MapPinned },
    { label: "Requests", to: "/driver/bookings", icon: CalendarRange },
    { label: "Alerts", to: "/notifications", icon: Bell, badge: unreadCount },
    { label: "Profile", to: "/driver/profile", icon: User },
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <header className="sticky top-0 z-30 hidden lg:block border-b border-border bg-surface/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          {/* Logo & Mode Switch Pill (Stitch UI #6) */}
          <div className="flex items-center gap-6">
            <Link to="/driver/dashboard" className="flex items-center gap-2.5">
              ...
              <span className="text-lg font-black tracking-tight text-foreground">
                ...
                <span className="ml-1.5 rounded-md bg-primary/10 px-1.5 py-0.5 text-[10px] font-extrabold text-primary">
                  DRIVER
                </span>
              </span>
            </Link>
            ...
            <div className="flex items-center rounded-full bg-surface-subtle p-1 border border-border text-xs font-extrabold shadow-inner">
              <button
                type="button"
                onClick={() => handleModeSwitch("RIDER")}
          ...
          <nav className="flex items-center gap-1">
            {desktopNavItems.map(({ label, to, icon: Icon, badge }) => {
                ...
                <div className="absolute right-0 top-11 w-48 rounded-2xl border border-border bg-surface p-2 shadow-premium backdrop-blur-md z-40 space-y-1 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      handleModeSwitch("RIDER");
                    }}
                    className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold text-foreground hover:bg-surface-subtle transition text-left"
                  >
      ...
      {/* ─── Mobile Header with Mode Toggle (Stitch UI #6) ─── */}
      ...
      {/* ─── Mobile Bottom Navigation (Stitch UI #6) ─── */}
      <nav className="fixed bottom-0 left-0 right-0 z-40 block lg:hidden border-t border-border bg-surface/95 backdrop-blur-md pb-[env(safe-area-inset-bottom)]">
        <div className="flex items-center justify-around px-2 py-2">
          {mobileNavItems.map(({ label, to, icon: Icon, badge }) => {
            const isActive = location.pathname === to;
            return (
              <NavLink
                key={to}
